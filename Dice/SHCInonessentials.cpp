...
void SHCIbasics::DoPerturbativeDeterministicOffdiagonal(
  ...
  std::vector<std::vector<std::vector<vector<Determinant> > > >
      hashedDetBeforeMPI(
  ...
  hashedEnergyBeforeMPI(commsize,
                        std::vector<std::vector<vector<double> > >(num_thrds));
    ...
    for (int k = localStart; k < detIndex.size(); k++) {
      ...
      for (int j = 0; j < k; j++) {
        ...
        if (Dets[J].connected(Dets[K]) ||
          ...
          if (DoRDM) orbDifference[K].push_back(orbDiff);
        }
      }
    }
  }
...
void SHCImakeHamiltonian::MakeHfromHelpers2(
    ...
    vector<vector<int> >& SinglesFromBeta, std::vector<Determinant>& Dets,
    int StartIndex, std::vector<std::vector<int> >& connections,
    std::vector<std::vector<CItype> >& Helements, int Norbs, oneInt& I1,
    twoInt& I2, double& coreE, std::vector<std::vector<size_t> >& orbDifference,
    bool DoRDM) {
  int proc = 0, nprocs = 1;
...
void SHCImakeHamiltonian::MakeSHMHelpers(
    ...
    vector<int*>& betaVecSHM, int*& alphaVecLenSHM, vector<int*>& alphaVecSHM) {
...
void SHCIbasics::DoPerturbativeStochastic2SingleListDoubleEpsilon2OMPTogether(
  ...
  for (int i = 0; i < ci.rows(); i++) cumulative += abs(ci(i, 0));
...
#pragma omp barrier
      ...
      for (size_t i = 0; i < Psi1.size(); i++) detIndex[i] = i;
...
void SHCIbasics::getDeterminants(
  ...
  for (int ij = 0; ij < nclosed * nclosed; ij++) {
    ...
    if (true &&
        (ints != I2hb.sameSpin.end() &&
         ints != I2hb.oppositeSpin.end())) {  // we have this pair stored in
                                              // heat bath integrals
...
void SHCIbasics::getDeterminants2Epsilon(
  ...
  std::map<Determinant,
           std::tuple<double, double, double, double, double> >::iterator
      det_it;
  for (int ia = 0; ia < nopen * nclosed; ia++) {
    int i = ia / nopen, a = ia % nopen;
    if (open[a] / 2 > schd.nvirt + nclosed / 2)
...
void SHCIbasics::DoPerturbativeStochastic2SingleListDoubleEpsilon2AllTogether(
  ...
  std::vector<std::vector<std::vector<vector<Determinant> > > >
      hashedDetBeforeMPI(
          mpigetsize(),
          std::vector<std::vector<vector<Determinant> > >(num_thrds));
  std::vector<std::vector<std::vector<vector<Determinant> > > >
      hashedDetAfterMPI(
          mpigetsize(),
          std::vector<std::vector<vector<Determinant> > >(num_thrds));
  ...
  std::vector<std::vector<std::vector<vector<double> > > > hashedNum2AfterMPI(
      mpigetsize(), std::vector<std::vector<vector<double> > >(num_thrds));
  std::vector<std::vector<std::vector<vector<double> > > >
  hashedEnergyBeforeMPI(mpigetsize(),
                        std::vector<std::vector<vector<double> > >(num_thrds));
  std::vector<std::vector<std::vector<vector<double> > > > hashedEnergyAfterMPI(
      mpigetsize(), std::vector<std::vector<vector<double> > >(num_thrds));
...
#pragma omp parallel
  {
    for (int iter = 0; iter < niter; iter++) {
      ...
      for (int i = 0; i < Nsample; i++) wts1[i] /= (num_thrds * mpigetsize());
      int Nmc = Nsample;
      double norm = 0.0;
