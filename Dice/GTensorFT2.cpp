...
void readInput(string input, vector<std::vector<int> >& occupied,
               schedule& schd);
double getdEusingDeterministicPT(vector<Determinant>& Dets,
                                 vector<MatrixXx>& ci, vector<double>& E0,
                                 oneInt& I1, twoInt& I2,
                                 twoIntHeatBathSHM& I2HB, vector<int>& irrep,
                                 schedule& schd, double coreE, int nelec);

void initDets(vector<MatrixXx>& ci, vector<Determinant>& Dets, schedule& schd,
              vector<vector<int> >& HFoccupied);
...
int main(int argc, char* argv[]) {
  ...
  string shciint2 =
      "SHCIint2" + to_string(static_cast<long long>(time(NULL) % 1000000));
  string shciint2shm =
      "SHCIint2shm" + to_string(static_cast<long long>(time(NULL) % 1000000));
  int2Segment = boost::interprocess::shared_memory_object(
      boost::interprocess::open_or_create, shciint2.c_str(),
      boost::interprocess::read_write);
  int2SHMSegment = boost::interprocess::shared_memory_object(
      boost::interprocess::open_or_create, shciint2shm.c_str(),
      boost::interprocess::read_write);
  ...
  if (Determinant::EffDetLen > DetLen) {
    cout << "change DetLen in global.h to " << Determinant::EffDetLen
         << " and recompile " << endl;
    exit(0);
  }
  ...
  for (int a = 1; a < norbs / 2 + 1; a++) {
    ...
    LplusS[1](2 * (a - 1), 2 * (a - 1) + 1) +=
        std::complex<double>(0, -ge / 2.);  // alpha beta
    LplusS[1](2 * (a - 1) + 1, 2 * (a - 1)) +=
        std::complex<double>(0, ge / 2.);  // beta alpha
  ...
  if (!schd.stochastic) {
    Ezero = getdEusingDeterministicPT(Dets, ci, E0, I1, I2, I2HBSHM, irrep,
                                      schd, coreE, nelec);
  } else
    Ezero = E0[0];
  ...
  for (int a = 0; a < 3; a++) {
    ...
    if (!schd.stochastic) {
      Bfpm[2 * a] = getdEusingDeterministicPT(Dets, ci, E0, I1, I2, I2HBSHM,
    ...
    if (!schd.stochastic) {
      Bfpm[2 * a + 1] = getdEusingDeterministicPT(Dets, ci, E0, I1, I2, I2HBSHM,
                                                  irrep, schd, coreE, nelec);
    } else
      Bfpm[2 * a + 1] = E0[0];
  ...
  for (int a = 0; a < 3; a++) {
    ...
    if (!schd.stochastic) {
      Sfpm[2 * a] = getdEusingDeterministicPT(Dets, ci, E0, I1, I2, I2HBSHM,
                                              irrep, schd, coreE, nelec);
    } else
      Sfpm[2 * a] = E0[0];
    ...
    if (!schd.stochastic) {
      Sfpm[2 * a + 1] = getdEusingDeterministicPT(Dets, ci, E0, I1, I2, I2HBSHM,
                                                  irrep, schd, coreE, nelec);
    } else
      Sfpm[2 * a + 1] = E0[0];
  ...
  for (int a = 0; a < 3; a++)
    for (int b = 0; b < a + 1; b++) {
      ...
      if (!schd.stochastic) {
        plusplus = getdEusingDeterministicPT(Dets, ci, E0, I1, I2, I2HBSHM,
                                             irrep, schd, coreE, nelec);
      } else
        plusplus = E0[0];
      ...
      if (!schd.stochastic) {
        minusminus = getdEusingDeterministicPT(Dets, ci, E0, I1, I2, I2HBSHM,
                                               irrep, schd, coreE, nelec);
      } else
        minusminus = E0[0];
      ...
      cout << plusplus << "  " << Bfpm[0] << "  " << Sfpm[0] << "  " << Ezero
           << "  " << Bfpm[1] << "  " << Sfpm[1] << "  " << minusminus << endl;
      Gtensor(a, b) = (plusplus - Bfpm[2 * a] - Sfpm[2 * b] + 2 * Ezero -
                       Bfpm[2 * a + 1] - Sfpm[2 * b + 1] + minusminus) /
                      2 / (epsilon * epsilon);
  ...
  cout << str(boost::format("g1= %9.6f,  shift: %6.0f\n") %
              pow(eigensolver.eigenvalues()[0], 1.0) %
              ((-ge + pow(eigensolver.eigenvalues()[0], 1.0)) * 1.e6));
  cout << str(boost::format("g2= %9.6f,  shift: %6.0f\n") %
              pow(eigensolver.eigenvalues()[1], 1.0) %
              ((-ge + pow(eigensolver.eigenvalues()[1], 1.0)) * 1.e6));
  cout << str(boost::format("g3= %9.6f,  shift: %6.0f\n") %
              pow(eigensolver.eigenvalues()[2], 1.0) %
              ((-ge + pow(eigensolver.eigenvalues()[2], 1.0)) * 1.e6));
...
double getdEusingDeterministicPT(vector<Determinant>& Dets,
                                 ...
                                 schedule& schd, double coreE, int nelec) {
  schd.doGtensor =
      false;  /// THIS IS DONE BECAUSE WE DONT WANT TO
              /// doperturbativedeterministicoffdiagonal to calculate rdm
  ...
  for (int root1 = 0; root1 < schd.nroots; root1++) {
    for (int root2 = root1 + 1; root2 < schd.nroots; root2++) {
      Heff(root1, root1) = 0.0;
      Heff(root2, root2) = 0.0;
      Heff(root1, root2) = 0.0;
      SHCIbasics::DoPerturbativeDeterministicOffdiagonal(
          Dets, ci[root1], E0[root1], ci[root2], E0[root2], I1, I2, I2HBSHM,
          irrep, schd, coreE, nelec, root1, Heff(root1, root1),
          Heff(root2, root2), Heff(root1, root2), spinRDM);
  ...
  // cout << eigensolver.eigenvalues()(1,0)<<"
  // "<<eigensolver.eigenvalues()(0,0)<<endl; exit(0);
