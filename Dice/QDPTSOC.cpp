...
int main(int argc, char* argv[]) {
  ...
  if (HFoccupied[0].size() % 2 != 0 && schd.Trev != 0) {
    pout << "Cannot use time reversal symmetry for odd electron system."
         << endl;
    schd.Trev = 0;
  }
  ...
  for (int i = 0; i < schd.prefix.size(); i++) {
    char file[5000];
    sprintf(file, "%s/%d-variational.bkp", schd.prefix[i].c_str(),
            mpigetrank());
    ...
    for (int j = 0; j < Localdiag.rows(); j++) load >> Localdiag(j, 0);
    load >> Localci;
    load >> LocalEnergy;
    ...
    if (i == 0) {
      ...
      ci.clear();
      ci.resize(bkpci.size() + Localci.size(),
                MatrixXd::Zero(Localci[0].rows() + bkpci[0].rows(), 1));
      for (int j = 0; j < bkpci.size(); j++)
        ci[j].block(0, 0, bkpci[0].rows(), 1) = 1. * bkpci[j];
      for (int j = 0; j < Localci.size(); j++) {
        ci[j + bkpci.size()].block(bkpci[0].rows(), 0, Localci[0].rows(), 1) =
            1. * Localci[j];
      }
    }
  ...
  int hsubspaceSize = 0;
  // S+S-
  for (int detIndex = ci[0].rows();
       detIndex < ci[0].rows() + SplusConnections.size(); detIndex++) {
    std::vector<size_t>& connections =
        SplusConnections[detIndex - ci[0].rows()];
  ...
  for (int j = 0; j < ci.size(); j++) {
    int s =
        static_cast<int>((-1.0 + pow(1 + 4 * SpinSquare[j], 0.5)) + 0.5);  // 2S
    Spin[j] = s;
    hsubspaceSize += s + 1;  // 2s+1
    if (j != 0) rowIndex[j] = rowIndex[j - 1] + Spin[j - 1] + 1;
  }

  cout << str(boost::format("PERFORMING QDPTSOC with %s roots\n") % ci.size());
  for (int j = 0; j < ci.size(); j++)
    cout << str(
        boost::format("State: %3d,  2S: %3d,  S^2: %8.2g,  dE: %10.2f\n") % j %
        Spin[j] % SpinSquare[j] % ((Energy[j] - Energy[0]) * 219470));
  ...
  SHCImakeHamiltonian::updateSOCconnections(Dets, 0, connections, orbDifference,
                                            Helements, norbs, I1, nelec);
  ...
  for (int i = 0; i < ci.size(); i++) {
    for (int j = i + 1; j < ci.size(); j++) {
      ...
      SOChelper::calculateMatrixElements(
          s1, s2, Sz, rowIndex[i], rowIndex[j], ci[i], ci[j], connections,
          Helements, Hsubspace, Dets, norbs, beginS0, beginSp, beginSm);
    }
  }
  Hsubspace *= 219470.;
  ...
  for (int i = 0; i < Hsubspace.rows(); i++)
    cout << "Root: " << i << " -> Energy: " << eigensolver.eigenvalues()[i]
         << endl;

  // NOW DO THE GTENSOR CALCULATION
  if (schd.doGtensor) {
    if (abs(eigensolver.eigenvalues()[0] - eigensolver.eigenvalues()[1]) >
        10.0) {
      cout << "Energy difference between kramer's pair greater than 10."
           << endl;
    ...
    for (int a = 0; a < 3; a++) {
      ...
      SHCImakeHamiltonian::updateSOCconnections(Dets, 0, connections,
                                                orbDifference, Helements, norbs,
                                                LplusSInt, nelec);
      ...
      for (int j = 0; j < ci.size(); j++) {                          // bra
        for (int i = j; i < ci.size(); i++) {                        // ket
          for (int sz2 = -Spin[j]; sz2 <= Spin[j]; sz2 += 2) {       // bra Sz
            for (int sz1 = -Spin[i]; sz1 <= Spin[i]; sz1 += 2) {     // ket
                                                                     // Sz
              ...
              // bra
              if (sz2 == -Spin[j])
                SOChelper::getSminus(ci[j], c2extended, Dets, beginS0, beginSp,
                                     beginSm, norbs);
              else
              ...
              // ket
              if (sz1 == -Spin[i])
                SOChelper::getSminus(ci[i], c1extended, Dets, beginS0, beginSp,
                                     beginSm, norbs);
              else
                c1extended.block(0, 0, ci[i].rows(), 1) = 1. * ci[i];

              H(c1extended, Hc1);
              LplusS[a](rowIndex[j] + (-sz2 + Spin[j]) / 2,
                        rowIndex[i] + (-sz1 + Spin[i]) / 2) =
                  (c2extended.adjoint() * Hc1)(0, 0);
              LplusS[a](rowIndex[i] + (-sz1 + Spin[i]) / 2,
                        rowIndex[j] + (-sz2 + Spin[j]) / 2) =
                  (Hc1.adjoint() * c2extended)(0, 0);
            }
          }
        }
      }
    ...
    for (int a = 0; a < 3; a++)
      Intermediate[a] =
          eigensolver.eigenvectors().block(0, 0, hsubspaceSize, 2).adjoint() *
          (LplusS[a] *
           eigensolver.eigenvectors().block(0, 0, hsubspaceSize, 2));
    ...
    for (int i = 0; i < 3; i++)
      for (int j = 0; j < 3; j++)
        Gtensor(i, j) +=
            2. * (Intermediate[i].adjoint() * Intermediate[j]).trace();
    ...
    cout << str(boost::format("g1= %9.6f,  shift: %6.0f\n") %
                pow(eigensolver.eigenvalues()[0], 0.5) %
                ((-ge + pow(eigensolver.eigenvalues()[0], 0.5)) * 1.e6));
    cout << str(boost::format("g2= %9.6f,  shift: %6.0f\n") %
                pow(eigensolver.eigenvalues()[1], 0.5) %
                ((-ge + pow(eigensolver.eigenvalues()[1], 0.5)) * 1.e6));
    cout << str(boost::format("g3= %9.6f,  shift: %6.0f\n") %
                pow(eigensolver.eigenvalues()[2], 0.5) %
                ((-ge + pow(eigensolver.eigenvalues()[2], 0.5)) * 1.e6));
  }
