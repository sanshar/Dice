...
int main(int argc, char* argv[]) {
  ...
  if (mpigetrank() == 0)
    readInput("input.dat", HFoccupied,
              schd);  // epsilon1, epsilon2, tol, num_thrds, eps, dE);
  ...
  char detchar[norbs];
  d.getRepArray(detchar);
  std::cout << Energy(detchar, norbs, I1, I2, coreE) << " " << coreE
            << std::endl;
  ...
  MatrixXd X0(dets.size(), 1);
  X0 *= 0.0;
  X0(0, 0) = 1.0;
  MatrixXd diag(dets.size(), 1);
  diag *= 0.0;
  char detChar[norbs * dets.size()];
  cout << dets.size() << endl;
  std::vector<std::vector<int> > connections(dets.size(),
                                             std::vector<int>(1, 0));
  std::vector<std::vector<double> > Helements(dets.size(),
...
#pragma omp parallel for schedule(dynamic)
  ...
  for (int i = 0; i < 5; i++) {
    ...
    int m = distance(
        &diagcopy(0, 0),
        max_element(&diagcopy(0, 0), &diagcopy(0, 0) + diagcopy.rows(), comp));
...
#pragma omp parallel for schedule(dynamic)
  for (size_t i = 0; i < dets.size(); i++) {
    ...
    for (size_t j = i; j < dets.size(); j++) {
      if (dets[i].connected(dets[j])) {
        double hij =
            Hij(&detChar[norbs * i], &detChar[norbs * j], Norbs, I1, I2, coreE);
  ...
  for (int i = 0; i < 5; i++) {
    compAbs comp;
    int m = distance(&X0(0, 0),
                     max_element(&X0(0, 0), &X0(0, 0) + X0.rows(), comp));
