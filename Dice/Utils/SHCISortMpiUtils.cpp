...
void RemoveDuplicates(std::vector<Determinant>& Det, std::vector<double>& Num1,
                      std::vector<double>& Num2, std::vector<double>& Energy,
...
std::vector<Determinant>& Det, std::vector<double>& Num1, std::vector<double>&
Num2, std::vector<double>& Energy, std::vector<char>& present) {
...
int partition(Determinant* A, int p, int q, CItype* pNum, double* Energy,
  ...
  for (j = p + 1; j < q; j++) {
    if (A[j] < x || A[j] == x) {
      ...
      if (present != NULL) swap(present->at(i), present->at(j));
      if (det_energy != NULL) swap(det_energy->at(i), det_energy->at(j));
    }
  }
  ...
  if (present != NULL) swap(present->at(i), present->at(p));
  if (det_energy != NULL) swap(det_energy->at(i), det_energy->at(p));
  return i;
}
...
void quickSortAll(Determinant* A, int p, int q, CItype* pNum, double* Energy,
  ...
  int r;
  if (p < q) {
    r = partitionAll(A, p, q, pNum, Energy, var_indices, orbDifference,
                     det_energy, present);
    quickSortAll(A, p, r, pNum, Energy, var_indices, orbDifference, det_energy,
                 present);
    quickSortAll(A, r + 1, q, pNum, Energy, var_indices, orbDifference,
                 det_energy, present);
  }
}

void merge(Determinant* a, long low, long high, long mid, long* x,
           Determinant* c, long* cx) {
  long i, j, k;
  i = low;
  k = low;
  j = mid + 1;
...
void mergesort(Determinant* a, long low, long high, long* x, Determinant* c,
               long* cx) {
  long mid;
  if (low < high) {
    mid = (low + high) / 2;
...
StitchDEH::StitchDEH() {
  ...
  extra_info = false;
  var_indices =
      boost::shared_ptr<vector<vector<int> > >(new vector<vector<int> >());
  orbDifference = boost::shared_ptr<vector<vector<size_t> > >(
      new vector<vector<size_t> >());
  var_indices_beforeMerge = boost::shared_ptr<vector<int> >(new vector<int>());
  orbDifference_beforeMerge =
      boost::shared_ptr<vector<size_t> >(new vector<size_t>());
}
...
StitchDEH::StitchDEH(boost::shared_ptr<vector<Determinant> > pD,
    ...
    : Det(pD),
      ...
      var_indices(pvar),
      orbDifference(porb),
      var_indices_beforeMerge(pvar_beforeMerge),
      orbDifference_beforeMerge(porb_beforeMerge) {
  extra_info = true;
};
...
void StitchDEH::QuickSortAndRemoveDuplicates() {
  ...
  if (extra_info) {
    quickSortAll(&(Det->operator[](0)), 0, Det->size(), &(Num->operator[](0)),
                 &(Energy->operator[](0)), &(var_indices->operator[](0)),
                 &(orbDifference->operator[](0)));
  } else {
    quickSort(&(Det->operator[](0)), 0, Det->size(), &(Num->operator[](0)),
              &(Energy->operator[](0)));
  }
...
void StitchDEH::MergeSort() {
  ...
  for (size_t i = 0; i < Detcopy.size(); i++) detIndex[i] = i;
  mergesort(&Detcopy[0], 0, Detcopy.size() - 1, &detIndex[0],
            &(Det->operator[](0)), &detIndexcopy[0]);
  detIndexcopy.clear();
  ...
  if (Num2->size() != 0) reorder(*Num2, detIndex);
  if (present->size() != 0) reorder(*present, detIndex);
  ...
  if (extra_info) {
    if (varIndicesEmtpy) {
      ...
      for (int i = 0; i < Vcopy.size(); i++) {
        ...
        var_indices->at(i).insert(var_indices->at(i).end(),
                                  Vcopy[detIndex[i]].begin(),
                                  Vcopy[detIndex[i]].end());
      }
      Vcopy.clear();
      vector<vector<size_t> > Ocopy = *orbDifference;
      for (int i = 0; i < Ocopy.size(); i++) {
        orbDifference->at(i).clear();
        orbDifference->at(i).insert(orbDifference->at(i).end(),
                                    Ocopy[detIndex[i]].begin(),
                                    Ocopy[detIndex[i]].end());
      }
      Ocopy.clear();
    }
  }
  ...
  if (extra_info) {
    if (varIndicesEmtpy) {
      var_indices->push_back(
          std::vector<int>(1, var_indices_beforeMerge->at(0)));
      orbDifference->push_back(
          std::vector<size_t>(1, orbDifference_beforeMerge->at(0)));
    }
  }
}
...
void StitchDEH::MergeSortAndRemoveDuplicates() {
  ...
  if (varIndicesEmtpy) {
    for (size_t i = 1; i < Detcopy.size(); i++) {
      if (!(Detcopy[i] == Detcopy[i - 1])) {
        ...
        if (Num2->size() != 0) Num2->operator[](uniqueSize) = Num2->at(i);
        if (present->size() != 0)
          present->operator[](uniqueSize) = present->at(i);
        Energy->operator[](uniqueSize) = Energy->at(i);
        if (extra_info) {
          var_indices->push_back(
              std::vector<int>(1, var_indices_beforeMerge->at(i)));
          orbDifference->push_back(
              std::vector<size_t>(1, orbDifference_beforeMerge->at(i)));
        ...
        if (abs(Energy->operator[](uniqueSize) - Energy->at(i)) > 1.e-10 &&
            abs(Energy->operator[](uniqueSize) - Energy->at(i)) >
                1.e-10 * abs(Energy->at(i))) {
          cout << uniqueSize << "  " << i << "  " << endl;
          cout << Detcopy[i] << endl;
          cout << Energy->at(uniqueSize) << "  " << Energy->at(i) << "  "
               << Detcopy[i - 1] << endl;
        ...
        if (extra_info) {
          ...
          orbDifference->at(uniqueSize)
              .push_back(orbDifference_beforeMerge->at(i));
  ...
  Det->resize(uniqueSize + 1);
  Num->resize(uniqueSize + 1);
  if (Num2->size() != 0) Num2->resize(uniqueSize + 1);
  if (present->size() != 0) present->resize(uniqueSize + 1);
  Energy->resize(uniqueSize + 1);
  if (extra_info) {
    var_indices->resize(uniqueSize + 1);
    orbDifference->resize(uniqueSize + 1);
...
void StitchDEH::RemoveDetsPresentIn(std::vector<Determinant>& SortedDets) {
  ...
  while (i < Detcopy.size() && vec_it != SortedDets.end()) {
    if (Detcopy[i] < *vec_it) {
      Det->operator[](uniqueSize) = Detcopy[i];
      Num->operator[](uniqueSize) = Numcopy[i];
      if (Num2->size() != 0) Num2->operator[](uniqueSize) = Num2copy[i];
      if (present->size() != 0)
  ...
  while (i < Detcopy.size()) {
    ...
    Num->operator[](uniqueSize) = Numcopy[i];
    if (Num2->size() != 0) Num2->operator[](uniqueSize) = Num2copy[i];
    if (present->size() != 0) present->operator[](uniqueSize) = presentcopy[i];
    Energy->operator[](uniqueSize) = Ecopy[i];
    if (extra_info) {
      var_indices->operator[](uniqueSize) = Vcopy[i];
      orbDifference->operator[](uniqueSize) = Ocopy[i];
  ...
  if (Num2->size() != 0) Num2->resize(uniqueSize);
  if (present->size() != 0)
    present->resize(uniqueSize);  // operator[](uniqueSize) =
                                  // presentcopy->at(i);
  Energy->resize(uniqueSize);
  if (extra_info) {
    var_indices->resize(uniqueSize);
    orbDifference->resize(uniqueSize);
  }
}
...
void StitchDEH::RemoveDuplicates() {
  ...
  for (size_t i = 1; i < Detcopy.size(); i++) {
    if (!(Detcopy[i] == Detcopy[i - 1])) {
      ...
      Det->operator[](uniqueSize) = Detcopy[i];
      Num->operator[](uniqueSize) = Numcopy[i];
      if (Num2->size() != 0) Num2->operator[](uniqueSize) = Num2->operator[](i);
      if (present->size() != 0)
        present->operator[](uniqueSize) = present->at(i);
      Energy->operator[](uniqueSize) = Ecopy[i];
      if (extra_info) {
  ...
  if (Num2->size() != 0) Num2->resize(uniqueSize + 1);
  if (present->size() != 0)
    present->resize(uniqueSize + 1);  // operator[](uniqueSize) =
                                      // present->at(i);
  Energy->resize(uniqueSize + 1);
  if (extra_info) {
    var_indices->resize(uniqueSize + 1);
...
void StitchDEH::deepCopy(const StitchDEH& s) {
  *Det = *(s.Det);
  *Num = *(s.Num);
  if (s.Num2->size() != 0) *Num2 = *(s.Num2);
  if (s.present->size() != 0) *present = *(s.present);
  *Energy = *(s.Energy);
  if (extra_info) {
...
void StitchDEH::merge(const StitchDEH& s) {
  ...
  while (j < Detcopy.size() && k < s.Det->size()) {
    if (Detcopy.operator[](j) < s.Det->operator[](k)) {
      ...
      if (Num2->size() != 0) Num2->operator[](l) = Num2copy.operator[](j);
      if (present->size() != 0)
