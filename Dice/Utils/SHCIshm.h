...
void SHMVecFromVecs(std::vector<T>& vec, T*& SHMvec, std::string& SHMname,
  ...
  if (comm_rank == 0) totalMemory = vec.size() * sizeof(T);
#ifndef SERIAL
  ...
  if (comm_rank == 0) {
    for (size_t i = 0; i < vec.size(); i++) SHMvec[i] = vec[i];
  }
#ifndef SERIAL
  MPI_Barrier(MPI_COMM_WORLD);
#endif
...
void SHMVecFromVecs(T* vec, int vecsize, T*& SHMvec, std::string& SHMname,
  ...
  if (comm_rank == 0) totalMemory = vecsize * sizeof(T);
#ifndef SERIAL
  MPI_Bcast(&totalMemory, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
#endif
  ...
  if (comm_rank == 0) {
    for (size_t i = 0; i < vecsize; i++) SHMvec[i] = vec[i];
  }
#ifndef SERIAL
  MPI_Barrier(MPI_COMM_WORLD);
#endif
