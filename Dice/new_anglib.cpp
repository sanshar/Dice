...
double nine_j(int na, int nb, int nc, int nd, int ne, int nf, int ng, int nh,
  ...
  if (na + nb < nc || abs(na - nb) > nc) return ninej;
  if (nd + ne < nf || abs(nd - ne) > nf) return ninej;
  if (ng + nh < ni || abs(ng - nh) > ni) return ninej;
  if (na + nd < ng || abs(na - nd) > ng) return ninej;
  if (nb + ne < nh || abs(nb - ne) > nh) return ninej;
  if (nc + nf < ni || abs(nc - nf) > ni) return ninej;
...
double six_j(int na, int nb, int nc, int nd, int ne, int nf) {
  // Initializing
  double sixj = 0.0;
  if ((na + nb) % 2 != nc % 2) return sixj;
  if ((nc + nd) % 2 != ne % 2) return sixj;
  if ((na + ne) % 2 != nf % 2) return sixj;
  if ((nb + nd) % 2 != nf % 2) return sixj;
  if (na + nb < nc || abs(na - nb) > nc) return sixj;
  if (nc + nd < ne || abs(nc - nd) > ne) return sixj;
  if (na + ne < nf || abs(na - ne) > nf) return sixj;
  if (nb + nd < nf || abs(nb - nd) > nf) return sixj;
