...
template <class T>
void reorder(vector<T>& A, std::vector<long>& reorder) {
  vector<T> Acopy = A;
...
int partition(Determinant* A, int p, int q, CItype* pNum, double* Energy,
              vector<double>* det_energy = NULL, vector<char>* present = NULL);
...
int partitionAll(Determinant* A, int p, int q, CItype* pNum, double* Energy,
                 ...
                 vector<double>* det_energy = NULL,
                 vector<bool>* present = NULL);
...
class StitchDEH {
 ...
 public:
  ...
  boost::shared_ptr<vector<CItype> >
      Num;  // the numerator for the first Var state
  boost::shared_ptr<vector<CItype> >
      Num2;  // the numberator for the second Var state (if present)
  boost::shared_ptr<vector<char> >
      present;  // the numberator for the second Var state (if present)
  ...
  // the next two store information about where a determinant in "C" arose from,
  // and takes into account the fact that a given determinant might have arisen
  // from multiple parent determinants in "V"
