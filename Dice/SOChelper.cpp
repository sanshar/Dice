...
void SOChelper::calculateSpinRDM(vector<MatrixXx>& spinRDM, MatrixXx& ci1,
                                 MatrixXx& ci2, vector<Determinant>& Dets1,
                                 int norbs, int nelec) {
  size_t Norbs = norbs;
  int num_thrds = omp_get_max_threads();

  map<Determinant, int> SortedDets;
  for (int i = 0; i < Dets1.size(); i++) SortedDets[Dets1[i]] = i;
...
#pragma omp parallel for
  for (int x = 0; x < Dets1.size(); x++) {
    ...
    for (int i = 0; i < nclosed; i++) {
      spinRDM_thrd[thrd][0](closed[i], closed[i]) +=
          conj(ci1(x, 0)) * ci1(x, 0);
      spinRDM_thrd[thrd][1](closed[i], closed[i]) +=
          conj(ci2(x, 0)) * ci2(x, 0);
      spinRDM_thrd[thrd][2](closed[i], closed[i]) +=
          conj(ci1(x, 0)) * ci2(x, 0);
    }
    ...
    for (int ia = 0; ia < nopen * nclosed; ia++) {
      ...
      if (it != SortedDets.end()) {
        int y = it->second;
        if (y != x) {
          spinRDM_thrd[thrd][0](open[a], closed[i]) +=
              conj(ci1(y, 0)) * ci1(x, 0) * sgn;
          spinRDM_thrd[thrd][1](open[a], closed[i]) +=
              conj(ci2(y, 0)) * ci2(x, 0) * sgn;
          spinRDM_thrd[thrd][2](open[a], closed[i]) +=
              conj(ci1(y, 0)) * ci2(x, 0) * sgn;
        }
      }
    }
  }
...
void SOChelper::doGTensor(vector<MatrixXx>& ci, vector<Determinant>& Dets,
  ...
  for (int a = 0; a < 3; a++)
    for (int i = 0; i < norbs; i++)
      for (int j = 0; j < norbs; j++) LplusS[a](i, j) += L[a](i, j);
  ...
  for (int a = 0; a < 3; a++) {
    ...
    pout << a << "  "
         << ((eigensolver.eigenvalues()[1] - eigensolver.eigenvalues()[0]) -
             ge) *
                1e6
         << endl;
  }
...
void SOChelper::doGTensor(vector<MatrixXx>& ci, vector<Determinant>& Dets,
  ...
  for (int a = 0; a < 3; a++) {
    ...
    // orbDifference, Helements, norbs, LplusS, nelec);
...
void SOChelper::getSplus(const MatrixXx& c2, MatrixXx& c2splus,
...
void SOChelper::getSminus(const MatrixXx& c2, MatrixXx& c2sminus,
                          vector<Determinant>& Dets,
                          vector<Determinant>::iterator& beginS0,
                          vector<Determinant>::iterator& beginSp,
                          vector<Determinant>::iterator& beginSm, int norbs) {
...
void SOChelper::calculateMatrixElements(
    int spin1, int spin2, int Sz, int rowIndex1, int rowIndex2,
  ...
  for (int Mop = -2; Mop <= 2; Mop += 2) {
    for (int ij = 0; ij < 9; ij++) {  // Sz2 Sz1
      ...
      // H(c2extended, Hc2);
      CItype element = (c1extended.adjoint() * Hc2)(0, 0) / c2extended.norm() /
                       c1extended.norm();
  ...
  for (int Sz1 = -spin1; Sz1 <= spin1; Sz1 += 2)
    for (int Sz2 = -spin2; Sz2 <= spin2; Sz2 += 2) {
      for (int Mop = -2; Mop <= 2; Mop += 2) {
        double cg = clebsch(spin2, Sz2, Jop, Mop, spin1, Sz1);
        Hsubspace(rowIndex1 + (-Sz1 + spin1) / 2,
                  rowIndex2 + (-Sz2 + spin2) / 2) += RME[(Mop + 2) / 2] * cg;
        if (rowIndex1 != rowIndex2)
          Hsubspace(rowIndex2 + (-Sz2 + spin2) / 2,
                    rowIndex1 + (-Sz1 + spin1) / 2) +=
              conj(RME[(Mop + 2) / 2] * cg);
      }
    }
  return;
...
void SOChelper::calculateMatrixElementsForgTensor(
    int spin1, int spin2, int Sz, int rowIndex1, int rowIndex2,
    const MatrixXx& c1, const MatrixXx& c2, vector<vector<int> >& connections,
