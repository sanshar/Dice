...
class twoInt {
 ...
 public:
  double* store;
  double maxEntry = 100.;  // JETS: This should be initialized to a value.
  MatrixXd Direct, Exchange;
  double zero = 0.0;  // JETS: Why are we not setting this to const 0.0?
  size_t norbs;
  bool ksym;
  // twoInt() : zero(0.0), maxEntry(100.) {} // JETS: Old way
  twoInt() {}
...
class twoIntHeatBath {
 public:
  ...
  double epsilon;
  double zero = 0.0;  // JETS: Same as above
  // twoIntHeatBath(double epsilon_) : zero(0.0), epsilon(fabs(epsilon_)) {}
  twoIntHeatBath(double epsilon_) : epsilon(fabs(epsilon_)) {}
  ...
  void constructClass(std::vector<int>& orbs, twoInt& I2, oneInt& I1,
                      int norbs) {
