...
private:
  ...
  template <class Archive>
  void serialize(Archive &ar, const unsigned int version) {
    ar &maxEntry &Direct &Exchange &zero &norbs &npair &ksym;
  }
...
public:
  ...
  inline double &operator()(int i, int j, int k, int l) {
    ...
    if (!ksym) {
      ...
      // unsigned int IJ = min(I,J) * norbs -(min(I,J) * (min(I,J) - 1)) / 2 +
      // max(I,J) - min(I,J); unsigned int KL = min(K,L) * norbs -(min(K,L)*
      // (min(K,L) - 1)) / 2 + max(K,L) - min(K,L);
      size_t IJ, KL;
      bool iv1 = false, iv2 = false;
      if (I < inner && J < inner)
        IJ = max(I, J) * (max(I, J) + 1) / 2 + min(I, J);
      else if (I >= inner && J >= inner)
        IJ = inner * (inner + 1) / 2 + inner * virt +
             max(I - inner, J - inner) * (max(I - inner, J - inner) + 1) / 2 +
             min(I - inner, J - inner);
      else {
        IJ = inner * (inner + 1) / 2 + inner * (max(I, J) - inner) + min(I, J);
        iv1 = true;
      }
      if (K < inner && L < inner)
        KL = max(K, L) * (max(K, L) + 1) / 2 + min(K, L);
      else if (K >= inner && L >= inner)
        KL = inner * (inner + 1) / 2 + inner * virt +
             max(K - inner, L - inner) * (max(K - inner, L - inner) + 1) / 2 +
             min(K - inner, L - inner);
      else {
        KL = inner * (inner + 1) / 2 + inner * (max(K, L) - inner) + min(K, L);
        iv2 = true;
      }
      size_t ind;
      if (iv1 && iv2)
        ind = niiii + niiiv + niivv +
              max(IJ - nii, KL - nii) * (max(IJ - nii, KL - nii) + 1) / 2 +
              min(IJ - nii, KL - nii);
      else
        ind = min(IJ, KL) * npair - (min(IJ, KL) * (min(IJ, KL) - 1)) / 2 +
              max(IJ, KL) - min(IJ, KL);
      // cout << I << "  " << J << "  " << K << "  " << L << "    " << IJ << "
      // " << KL << "    " << ind << endl;
      return store[ind];
...
public:
  ...
  // are the second pair if the integrals are for the same spin you have to
  // separately store (ia|jb) and (ib|ja) for opposite spin you just have to
  // store for a>b because the integral is (ia|jb) - (ib|ja) now this class is
  // made by just considering integrals that are smaller than threshhold
  ...
  std::map<std::pair<short, short>,
           std::multimap<float, std::pair<short, short>, compAbs>>
      oppositeSpin;
  std::map<std::pair<short, short>, std::multimap<float, short, compAbs>>
      singleIntegrals;
  ...
  // typically these can be all orbitals of the problem or just the active space
  // ones ab will typically contain all orbitals(norbs)
  void constructClass(std::vector<int> &orbs, twoInt &I2, oneInt &I1, int ncore,
                      int nact, bool cas = false) {
    ...
    for (int i = 0; i < s; i++) {
      for (int j = 0; j <= i; j++) {
        ...
        for (int a = start; a < end; a++) {
          if (fabs(I2(2 * i, 2 * j, 2 * a, 2 * a) -
                   I2(2 * i, 2 * a, 2 * a, 2 * j)) > epsilon)
            singleIntegrals[IJ].insert(pair<float, short>(
                I2(2 * i, 2 * j, 2 * a, 2 * a) - I2(2 * i, 2 * a, 2 * a, 2 * j),
                2 * a));

          if (fabs(I2(2 * i, 2 * j, 2 * a + 1, 2 * a + 1)) > epsilon)
            singleIntegrals[IJ].insert(pair<float, short>(
                I2(2 * i, 2 * j, 2 * a + 1, 2 * a + 1), 2 * a + 1));
        }
      }
    }
        ...
        for (int a = ncore; a < first_virtual; a++) {
          for (int b = ncore; b < first_virtual; b++) {
            ...
            if (fabs(I2(2 * i, 2 * a, 2 * j, 2 * b)) > epsilon)
              oppositeSpin[IJ].insert(pair<float, std::pair<short, short>>(
                  I2(2 * i, 2 * a, 2 * j, 2 * b), make_pair(a, b)));
            // samespin
            if (a >= b && fabs(I2(2 * i, 2 * a, 2 * j, 2 * b) -
                               I2(2 * i, 2 * b, 2 * j, 2 * a)) > epsilon) {
              sameSpin[IJ].insert(pair<float, std::pair<short, short>>(
                  I2(2 * i, 2 * a, 2 * j, 2 * b) -
                      I2(2 * i, 2 * b, 2 * j, 2 * a),
                  make_pair(a, b)));
              // sameSpin[IJ][fabs(I2(2*i,2*a,2*j,2*b) - I2(2*i,2*b,2*j,2*a))] =
...
public:
  ...
  // for each pair i,j it has sum_{a>b} abs((ai|bj)-(aj|bi)) if they are same
  // spin and sum_{ab} abs(ai|bj) if they are opposite spins
...
void readDQMCIntegralsRG(string fcidump, int &norbs, int &nalpha, int &nbeta,
                         double &ecore, Eigen::MatrixXd &h1,
                         Eigen::MatrixXd &h1Mod,
                         std::vector<Eigen::Map<Eigen::MatrixXd>> &chol,
                         std::vector<Eigen::Map<Eigen::MatrixXd>> &cholMat,
                         bool ghf = false);

void readDQMCIntegralsU(
    string fcidump, int &norbs, int &nalpha, int &nbeta, double &ecore,
    std::array<Eigen::MatrixXd, 2> &h1, std::array<Eigen::MatrixXd, 2> &h1Mod,
    std::vector<std::array<Eigen::Map<Eigen::MatrixXd>, 2>> &chol);

void readDQMCIntegralsSOC(string fcidump, int &norbs, int &nelec, double &ecore,
