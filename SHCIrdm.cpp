...
void SHCIrdm::EvaluateOneRDM(vector<vector<int>> &connections,
                             vector<Determinant> &Dets, MatrixXx &cibra,
                             MatrixXx &ciket,
                             vector<vector<size_t>> &orbDifference, int nelec,
  ...
  for (int i = 0; i < Dets.size(); i++) {
    ...
    for (int j = 1; j < connections[i / commsize].size(); j++) {
      ...
      if (orbDifference[i / commsize][j] / norbs / norbs ==
        ...
        s1RDM(c0, d0) +=
            sgn * localConj::conj(cibra(connections[i / commsize][j], 0)) *
            ciket(i, 0);
      }
    } // end j
  }   // end i
    ...
    for (int k = 0; k < DetsSize; k++) {
      ...
      // D=2
        ...
        for (int x = 0; x < nelec; x++) {
          cs[2] = closed[x];
          ds[0] = closed[x];
          if (closed[x] == ds[1] || closed[x] == ds[2])
            continue;
      ...
      // D=1
        ...
        for (int x = 0; x < nelec; x++) {
          cs[1] = closed[x];
          ds[1] = closed[x];
          if (closed[x] == cs[0] || closed[x] == ds[2])
            continue;
          for (int y = 0; y < x; y++) {
            cs[2] = closed[y];
            ds[0] = closed[y];
            if (closed[y] == ds[1] || closed[y] == ds[2])
              continue;
    ...
    for (int k = 0; k < DetsSize; k++) {
      ...
      // D=2
        ...
        for (int w = 0; w < nelec; w++) {
          cs[2] = closed[w];
          ds[1] = closed[w];
          if (closed[w] == cs[0] || closed[w] == cs[1])
            continue;
          if (closed[w] == ds[3] || closed[w] == ds[2])
            continue;
          for (int x = 0; x < w; x++) {
            cs[3] = closed[x];
            ds[0] = closed[x];
            if (closed[x] == cs[0] || closed[x] == cs[1])
              continue;
            if (closed[x] == ds[3] || closed[x] == ds[2])
              continue;
      ...
      // D=1
        ...
        for (int w = 0; w < nelec; w++) {
          cs[1] = closed[w];
          ds[2] = closed[w];
          if (closed[w] == cs[0] || closed[w] == ds[3])
            continue;
          for (int x = 0; x < w; x++) {
            cs[2] = closed[x];
            ds[1] = closed[x];
            if (closed[x] == cs[0] || closed[x] == ds[3])
              continue;
            for (int y = 0; y < x; y++) {
              cs[3] = closed[y];
              ds[0] = closed[y];
              if (closed[y] == cs[0] || closed[y] == ds[3])
                continue;
