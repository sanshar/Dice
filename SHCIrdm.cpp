...
void SHCIrdm::save3RDM(schedule& schd, MatrixXx& threeRDM, MatrixXx& s3RDM, int root, size_t norbs ) {
        ...
        for (int n5=0; n5 < nSpatOrbs; n5++) {
          if ( abs( s3RDM(n0*nSpatOrbs2+n1*nSpatOrbs+n2,
            ...
            ofs << str(boost::format("%3d   %3d   %3d   %3d   %3d   %3d   %20.14e\n")
                       %n0%n1%n2%n3%n4%n5
                       %s3RDM(n0*nSpatOrbs2+n1*nSpatOrbs+n2,n3*nSpatOrbs2+n4*nSpatOrbs+n5));
      }
      ofs.close();
    }
        ...
        for (int d3=0; d3 < n; d3++) {
          if ( abs( s4RDM( n3*c0+n2*c1+n*c2+c3,
            ...
            ofs << str(boost::format("%3d   %3d   %3d   %3d   %3d   %3d   %3d   %3d   %20.14e\n")
                       ...
                       %s4RDM( n3*c0+n2*c1+n*c2+c3, n3*d0+n2*d1+n*d2+d3 ) );
      }
      ofs.close();
    }
...
void SHCIrdm::UpdateRDMResponsePerturbativeDeterministic(
        Determinant *Dets, int DetsSize, CItype *ci,
        double& E0, oneInt& I1, twoInt& I2, schedule& schd,
...
void SHCIrdm::populateSpatialRDM(int& i, int& j, int& k, int& l,
                                 MatrixXx& s2RDM, CItype value, int& nSpatOrbs) {
  ...
  for (int i=0; i<DetsSize; i++) {
    ...
    for (int j=1; j<connections[i/commsize].size(); j++) {
      ...
      if (orbDifference[i/commsize][j]/norbs/norbs == 0) { //only single excitation
        ...
        int c1=(orbDifference[i/commsize][j]/norbs/norbs/norbs)%norbs;
        double sgn = 1.0;
...
double SHCIrdm::ComputeEnergyFromSpinRDM(int norbs, int nelec,
                                         oneInt& I1, twoInt& I2, double coreE, MatrixXx& twoRDM) {
...
void SHCIrdm::Evaluate3RDM(Determinant*  Dets, int DetsSize,
                           CItype* cibra, CItype* ciket, int nelec,
                           schedule& schd, int root, MatrixXx& threeRDM, MatrixXx& s3RDM) {
  /*
      ...
      // D=1
        ...
        for ( int x=0; x<nelec; x++ ) {
          ...
          for ( int y=0; y<x; y++ ) {
            ...
            // Gamma = c0 c1 c2 d0 d1 d2
            double sgn = 1.0;
            DetsK.parity( min(cs[0],ds[2]), max(cs[0],ds[2]), sgn ); // TODO Update repop order
...
void popSpin4RDM( vector<int>& cs, vector<int>& ds, CItype value, int& norbs, MatrixXx& fourRDM ){
...
void SHCIrdm::popSpatial4RDM( vector<int>& cs, vector<int>& ds, CItype value, int& nSOs, MatrixXx& s4RDM ){
  ...
  do {
    do {
      ...
      if (cs[cI[0]]%2==ds[dI[3]]%2 && cs[cI[1]]%2==ds[dI[2]]%2 &&
        ...
        s4RDM( gen4Idx(cs[cI[0]]/2,cs[cI[1]]/2, cs[cI[2]]/2,cs[cI[3]]/2, nSOs),
               gen4Idx(ds[dI[0]]/2,ds[dI[1]]/2, ds[dI[2]]/2,ds[dI[3]]/2, nSOs) ) += par*value;
      }
      ctr++;
    } while ( next_permutation( dI,dI+4 ) );
  } while ( next_permutation( cI, cI+4 ) );
...
void SHCIrdm::Evaluate4RDM(Determinant* Dets, int DetsSize,
                           CItype* cibra, CItype* ciket, int nelec,
                           schedule& schd, int root, MatrixXx& fourRDM, MatrixXx& s4RDM){
  /*
     TODO optimize speed and memory
  */
#ifndef SERIAL
    ...
    for (int k=0; k<DetsSize; k++) {
      ...
      // D=3
        ...
        for ( int w=0; w<nelec; w++) {
          ...
          if ( closed[w]==cs[0] || closed[w]==cs[1] || closed[w]==cs[2] ) continue;
          if ( closed[w]==ds[3] || closed[w]==ds[2] || closed[w]==ds[1] ) continue;
      ...
      // D=0
        ...
        cs.push_back(0); cs.push_back(0); cs.push_back(0); cs.push_back(0);
        ds.push_back(0); ds.push_back(0); ds.push_back(0); ds.push_back(0);
        for (int w=0; w<nelec; w++) {
