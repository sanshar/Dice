...
boost::interprocess::shared_memory_object DetsResponseSegment;
boost::interprocess::mapped_region regionDetsResponse;
std::string shciDetsResponse;
...
void initSHM() {
  ...
  shciDetsResponse = "SHCIDetsResponseshm" + to_string(static_cast<long long>(time(NULL) % 1000000));
  ...
  DetsResponseSegment = boost::interprocess::shared_memory_object(boost::interprocess::open_or_create, shciDetsResponse.c_str(), boost::interprocess::read_write);
...
void removeSHM(){
  ...
  boost::interprocess::shared_memory_object::remove(shciDetsResponse.c_str());
