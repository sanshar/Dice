  ...
  void operator()(CItype* x, CItype* y) {
    ...
    */
    ...
    if (numDets > 100000000) {  // more than 10 million
      ...
      for (size_t i = 0; i < sparseHam.connections.size(); i++) {
        for (size_t j = 0; j < sparseHam.connections[i].size(); j++) {
      ...
      for (int r = 0; r < localsize; r++) {
        ...
        if (localrank == r) {
          for (size_t i = 0; i < sparseHam.connections.size(); i++) {
            for (size_t j = 1; j < sparseHam.connections[i].size(); j++) {
              CItype hij = sparseHam.Helements[i][j];
              int J = sparseHam.connections[i][j];
#ifdef Complex
      ...
      for (size_t i = 0; i < sparseHam.connections.size(); i++) {
        for (size_t j = 0; j < sparseHam.connections[i].size(); j++) {
          CItype hij = sparseHam.Helements[i][j];
          size_t J = sparseHam.connections[i][j];
...
struct HmultDirect {
  ...
  void operator()(CItype* x, CItype* y) {
    ...
    // size_t norbs = Norbs; // Unused -JETS

    // diagonal element
    for (int k = StartIndex; k < DetsSize; k++) {
