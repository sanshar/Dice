...
#ifndef Complex
//=============================================================================
void LCC::doLCC(Determinant* Dets, CItype* ci, int DetsSize, double& E0,
                oneInt& I1, twoInt& I2, twoIntHeatBathSHM& I2HB,
                vector<int>& irrep, schedule& schd, double coreE, int nelec,
                int root) {
  //-----------------------------------------------------------------------------
  /*!
  BM_description
  ...
  determinants vector<int>& irrep: Irrep of the orbitals schedule& schd: The
  schedule double coreE: The core energy int nelec: Number of electrons int
  root: (unused)
  */
//-----------------------------------------------------------------------------
#ifndef SERIAL
  ...
  for (int iclass = 0; iclass < 8; iclass++) {
    ...
    for (int i = 0; i < DetsSize; i++) {
      ...
      LCC::getDeterminantsLCC(
          ...
          nelec, class_cor[iclass], class_act[iclass], class_vir[iclass]);
    }
      ...
      if (rank % ipow(2, level + 1) == 0 && rank + ipow(2, level) < size) {
        ...
        if (totalMemory != 0) {
          ...
          for (int i = 0; i < (numDets / maxint); i++)
            MPI_Recv(&(uniqueDEH.Num->at(oldSize)) + i * maxint, maxint,
                     MPI_DOUBLE, getproc, getproc, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
          MPI_Recv(&(uniqueDEH.Num->at(oldSize)) + (numDets / maxint) * maxint,
                   numDets - (numDets / maxint) * maxint, MPI_DOUBLE, getproc,
                   getproc, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
          for (int i = 0; i < (numDets / maxint); i++)
            MPI_Recv(&(uniqueDEH.Energy->at(oldSize)) + i * maxint, maxint,
                     MPI_DOUBLE, getproc, getproc, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
          MPI_Recv(
              &(uniqueDEH.Energy->at(oldSize)) + (numDets / maxint) * maxint,
              numDets - (numDets / maxint) * maxint, MPI_DOUBLE, getproc,
              getproc, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
          uniqueDEH.MergeSortAndRemoveDuplicates();
        }
      } else if (rank % ipow(2, level + 1) == 0 &&
                 rank + ipow(2, level) >= size) {
        continue;
        ...
        if (totalMemory != 0) {
          ...
          MPI_Send(&(uniqueDEH.Det->at(0)) + (totalMemory / maxint) * maxint,
                   ...
                   toproc, proc, MPI_COMM_WORLD);
          for (int i = 0; i < (numDets / maxint); i++)
            MPI_Send(&(uniqueDEH.Num->at(0)) + i * maxint, maxint, MPI_DOUBLE,
                     toproc, proc, MPI_COMM_WORLD);
          MPI_Send(&(uniqueDEH.Num->at(0)) + (numDets / maxint) * maxint,
                   numDets - (numDets / maxint) * maxint, MPI_DOUBLE, toproc,
                   proc, MPI_COMM_WORLD);
          for (int i = 0; i < (numDets / maxint); i++)
            MPI_Send(&(uniqueDEH.Energy->at(0)) + i * maxint, maxint,
    ...
    for (int i = 0; i < nDets; i++) VPsi0(i, 0) = uniqueDEH.Num->at(i);
    ...
    if (schd.DavidsonType != DIRECT)
      sparseHab.makeFromHelper(helpers, &Dets[0], 0, nDets, Determinant::norbs,
                               I1, I2, coreE, false);
    ...
    if (commrank == 0)
      cout << "Class " << iclass + 1
           << format(" [%3i %3i %3i] %20.9e    (%8i determinants, %7.2fsec)") %
                  (class_cor[iclass]) % (class_act[iclass]) %
                  (class_vir[iclass]) % (-ept) % (nDets) % (tB - tA)
           << endl;
  ...
  SHCImakeHamiltonian::SparseHam sparseHab;
  if (schd.DavidsonType != DIRECT)
    sparseHab.makeFromHelper(helpers, &Psi1Dets[0], 0, nDets,
  ...
  for (int iclass = 0; iclass < 8; iclass++) {
    cout << format("Class: %5i %8i %8i ==============================") %
                (iclass + 1) % (istart) % (istop)
         << endl;

    MatrixXx CoefA = MatrixXx::Zero(nDets, 1);
    for (int i = istart; i < istop; i++) CoefA(i, 0) = Psi1Coef[i];
    ...
    for (int jclass = 0; jclass < 8; jclass++) {
      if (jclass > iclass) {
        cout << format("  Class: %3i %8i %8i") % (jclass + 1) % (jstart) %
                    (jstop);

        MatrixXx CoefB = MatrixXx::Zero(nDets, 1);
        for (int i = jstart; i < jstop; i++) CoefB(i, 0) = Psi1Coef[i];
  ...
  double tB = getTime();
  cout << "\nTotal PT3             "
       << format("%20.9e  (%7.2fsec)") % (totalpt3) % (tB - tA) << endl;
  ...
  // Scenario2: all-in-one
  {
    ...
    for (int iclass = 0; iclass < 8; iclass++) {
      for (int i = start; i < stop; i++)
        for (int j = 0; j < sparseHab.connections[i].size(); j++) {
          if (sparseHab.connections[i][j] >= start &&
              sparseHab.connections[i][j] < stop)
    ...
    MatrixXx CoefA = MatrixXx::Zero(nDets, 1);
    for (int i = 0; i < nDets; i++) CoefA(i, 0) = Psi1Coef[i];

    MatrixXx HPsi1Class = MatrixXx::Zero(nDets, 1);
    Hab(&CoefA(0, 0), &HPsi1Class(0, 0));

    double totalpt3 = 0.0;
    for (int i = 0; i < nDets; i++) totalpt3 += CoefA(i, 0) * HPsi1Class(i, 0);
  ...
  */
  ...
  for (int ia = 0; ia < nopen * nclosed; ia++) {
    ...
    if (d_cor != class_cor || d_act != class_act || d_vir != class_vir) {
      // cout<<format("BM i: %3i %3i a: %3i %3i") %(i) %(closed[i]) %(a)
      // %(open[a]);
    ...
    if (fabs(integral) > epsilon) {
      ...
      // cout << "BM |D_a> "<<di<<" "<<format("%3i %3i %3i") %(d_cor) %(d_act)
      // %(d_vir)<<endl;
  ...
  for (int ij = 0; ij < nclosed * nclosed; ij++) {
    ...
    for (size_t index = start; index < end; index++) {
      ...
      LCC::get_landscape(closed[i], closed[j], a, b, &d_cor, &d_act, &d_vir,
                         schd);
      if (d_cor != class_cor || d_act != class_act || d_vir != class_vir) {
        // cout<<format("BM i: %3i %3i j: %3i %3i a: %3i b: %3i") %(i)
        // %(closed[i]) %(j) %(closed[j]) %(a) %(b);
        //    <<format(" landscape: %3i %3i %3i") %(d_cor) %(d_act) %(d_vir);
        //    <<" pass"<<endl;
        continue;
      } else {
...
void LCC::get_landscape(int i, int a, int* d_cor, int* d_act, int* d_vir,
                        schedule schd) {
  *d_cor = 0;
  *d_act = 0;
  *d_vir = 0;
  if (i < 2 * schd.ncore && a < 2 * (schd.ncore + schd.nact)) {
    ...
    *d_vir = +1;
    ...
    cout << "BM: what?? " << i << " " << a << " " << schd.ncore << " "
         << schd.nact << endl;
  }
}

void LCC::get_landscape(int i, int j, int a, int b, int* d_cor, int* d_act,
                        int* d_vir, schedule schd) {
  *d_cor = 0;
  *d_act = 0;
  *d_vir = 0;
  // a act and b act
  if (i < 2 * schd.ncore && j < 2 * schd.ncore &&
      a < 2 * (schd.ncore + schd.nact) && b < 2 * (schd.ncore + schd.nact)) {
    *d_cor = -2;
    *d_act = +2;
  } else if (i >= 2 * schd.ncore && j < 2 * schd.ncore &&
    ...
    *d_vir = +2;
    ...
    cout << "BM: what?? " << i << " " << j << " " << a << " " << b << " "
         << schd.ncore << " " << schd.nact << endl;
  }
}
#endif
