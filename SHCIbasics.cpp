...
void SHCIbasics::DoPerturbativeDeterministicOffdiagonal(vector<Determinant>& Dets, MatrixXx& ci1, double& E01,
  ...
  double energyEN = 0.0;
  boost::shared_ptr<vector<Determinant> > Detcz;
  boost::shared_ptr<vector<CItype> > Numcz;       //the numerator for the first Var state
  boost::shared_ptr<vector<CItype> > Num2cz;      //the numberator for the second Var state (if present)
  boost::shared_ptr<vector<char> > presentcz;      //the numberator for the second Var state (if present)
  boost::shared_ptr<vector<double> > Energycz;

  CItype totalPT1 = 0.0, totalPT2=0.0, totalPT12=0.0;
  ...
  for (int i=0; i<Dets.size(); i++) {
      ...
      SHCIgetdeterminants::getDeterminantsDeterministicPTWithSOC(Dets[i], i, abs(schd.epsilon2/ci1(i,0)), ci1(i,0),
						      ...
						      *Detcz, *Numcz, *Num2cz, *Energycz, schd, nelec);
  }

    
    vector<Determinant>& hasHEDDets = *Detcz;
    vector<CItype>& hasHEDNumerator = *Numcz;
    vector<CItype>& hasHEDNumerator2 = *Num2cz;
    vector<double>& hasHEDEnergy = *Energycz;
