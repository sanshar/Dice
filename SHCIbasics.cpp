    ...
    twoIntHeatBathSHM &I2HB, vector<int> &irrep, schedule &schd, double coreE, int nelec, int root)
{
  ...
  pout << endl
       << format("%6s  %14s  %5s %14s %10s  %10s") % ("Iter") % ("EPTcurrent") % ("State") % ("EPTavg") % ("Error") % ("Time(s)") << endl;
  ...
  for (int iter = 0; iter < niter; iter++)
  {
    ...
    if (commrank == 0)
    {
      ...
      if (currentIter < 5)
        std::cout << format("%6i  %14.8f  %5i %14.8f %10s  %10.2f") % (currentIter) % (E0 - finalE + finalELargeEps + EptLarge) % (root) % (E0 + AvgenergyEN / currentIter) % "--" % (getTime() - startofCalc);
      else
        std::cout << format("%6i  %14.8f  %5i %14.8f %10.2e  %10.2f") % (currentIter) % (E0 - finalE + finalELargeEps + EptLarge) % (root) % (E0 + AvgenergyEN / currentIter) % stddev % (getTime() - startofCalc);
      pout << endl;
    }
    ...
    if (stddev < schd.targetError)
    {
      ...
      pout << "epsilon2: " << schd.epsilon2 << endl
           ...
           << format("%8.2e") % (stddev) << endl
           << "Time(s):  " << getTime() - startofCalc << endl;
      break;
    }
  }
  return AvgenergyEN;
                                               ...
                                               bool appendPsi1ToPsi0)
{
  ...
  */
  ...
  if (commrank == 0)
  {
    ...
    pout << "epsilon2: " << schd.epsilon2 << endl
         << "PTEnergy: " << E0 + finalE << endl
         << "Time(s):  " << getTime() - startofCalc << endl;
  }

  if (schd.doResponse || schd.DoRDM)
  { //build RHS for the lambda equation
...
vector<double> SHCIbasics::DoVariational(vector<MatrixXx> &ci, vector<Determinant> &Dets, schedule &schd,
                                         twoInt &I2, twoIntHeatBathSHM &I2HB, vector<int> &irrep, oneInt &I1, double &coreE, int nelec, bool DoRDM, bool BruteForce)
{
  ...
  for (int iter = iterstart; iter < schd.epsilon1.size(); iter++)
  {
    ...
    MatrixXx Ham = MatrixXx::Zero(DetsSize, DetsSize);
    for (int i=0; i<DetsSize; i++) {
      for (int j=0; j<DetsSize; j++) {
        size_t orbDiff;
        if (i==j) Ham(i,j) = Dets[i].Energy(I1,I2,coreE);
        else Ham(i,j) = Hij(Dets[j], Dets[i], I1, I2, coreEbkp, orbDiff);
      }
    }
    ...
    if (commrank == 0) {
      ...
      char file2 [5000];
      sprintf (file2, "Hamiltonian.txt", schd.prefix[0].c_str());
      std::ofstream ofs2(file2, std::ios::out);

      for (int i=0; i<DetsSize; i++) {
        for (int j=i; j<DetsSize; j++) {
          int dist = Dets[i].ExcitationDistance(Dets[j]);
          ofs1 << str(boost::format("%3d %3d %12.8d %12.8d %3d") % (i+1) %(j+1) % HamFromHelper(i,j).real() % HamFromHelper(i,j).imag() % dist) << "  Dets[i]"<< Dets[i] << " Dets[j]"<< Dets[j] << endl;
          ofs2 << str(boost::format("%3d %3d %12.8d %12.8d %3d") % (i+1) %(j+1) % Ham(i,j).real() % Ham(i,j).imag() % dist)<< "  Dets[i]"<< Dets[i] << " Dets[j]"<< Dets[j] << endl;
        }
      }
    ...
    pout << format("%4i %4i  %10.2e  %10.2e") % (iter) % (0) % schd.epsilon1[iter] % (DetsSize);
