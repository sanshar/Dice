...
void CPSGHFSlater::readDefault() {
  ...
  GHFOrbs = MatrixXd::Zero(2*norbs, 2*norbs);
  MatrixXd GHFOrbsCopy = MatrixXd::Zero(2*norbs, 2*norbs);
  readHF(GHFOrbs, GHFOrbsCopy, schd.uhf);
...
void CPSGHFSlater::initWalker(GHFWalker& walk) {
      ...
      for (int i = 0; i < nalpha; i++)
		{
			...
			{
				if (abs(GHFOrbs(i, j)) > maxovlp && !d.getoccA(j))
				{
					maxovlp = abs(GHFOrbs(i, j));
					bestorb = j;
				}
			}
		...
		for (int i = 0; i < nbeta; i++)
		{
			...
			for (int j = 0; j < norbs; j++)
			{
				if (abs(GHFOrbs(i+norbs, j)) > maxovlp && !d.getoccB(j))
				{
					bestorb = j;
					maxovlp = abs(GHFOrbs(i+norbs, j));
				}
			}
...
void CPSGHFSlater::initWalker(GHFWalker& walk, Determinant& d) {

  walk.d = d;
	walk.initUsingWave(*this);
}

//void CPSGHFSlater::getDetMatrix(Determinant &d, Eigen::MatrixXd &DetAlpha, Eigen::MatrixXd &DetBeta)
//{
...
double CPSGHFSlater::getOverlapWithDeterminants(GHFWalker &walk)
{
...
double CPSGHFSlater::getOverlapFactor(int I, int J, int A, int B, GHFWalker& walk, bool doparity) {
  Determinant dcopy = walk.d;
  dcopy.setocc(I, false);
...
double CPSGHFSlater::getJastrowFactor(int i, int a, Determinant &dcopy, Determinant &d)
{
                                    ...
                                    VectorXd &grad)
{
  ...
  {
    grad(k + numJastrowVariables) += walk.GHFDet[k] / detovlp;
  }
...
void CPSGHFSlater::printVariables()
{
  cps.printVariables();
...
void CPSGHFSlater::updateVariables(Eigen::VectorXd &v)
{
  int norbs = Determinant::norbs;
  ...
  for (int i = 0; i < 2*norbs; i++)
  {
    for (int j = 0; j < 2*norbs; j++)
    {
        GHFOrbs(i, j) = v[numVars + 2 * i * norbs + j];
    }
  }
}
...
void CPSGHFSlater::getVariables(Eigen::VectorXd &v)
{
  int norbs = Determinant::norbs;
  ...
  for (int i = 0; i < 2*norbs; i++)
  {
    for (int j = 0; j < 2*norbs; j++)
    {
        v[numVars + 2 * i * norbs + j] = GHFOrbs(i, j);
    }
  }
}


long CPSGHFSlater::getNumJastrowVariables()
{
  return cps.getNumVariables();
}
//factor = <psi|w> * prefactor;

long CPSGHFSlater::getNumVariables()
{
  int norbs = Determinant::norbs;
  long numVars = 0;
  numVars += getNumJastrowVariables();
...
//<psi_t| (H-E0) |D>
void CPSGHFSlater::HamAndOvlp(GHFWalker &walk,
      ...
      for (int a = 0; a < open.size(); a++)
      {
        if (closed[i] % 2 == open[a] % 2 && abs(I2hb.Singles(closed[i], open[a])) > THRESH)
        {
          ...
          if (abs(tia) > THRESH)
          {
            ...
            //cout << Alpha << "  " << i << "  " << a << "  " << ovlpdetcopy << endl;
            ham += ovlpdetcopy * tia;
...
void CPSGHFSlater::derivativeOfLocalEnergy (GHFWalker &walk,
