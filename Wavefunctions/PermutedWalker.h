...
*/
#ifndef PermutedWalker_HEADER_H
#define PermutedWalker_HEADER_H
...
public:
  ...
  // for deterministic
  PermutedWalker(Jastrow &corr, const Slater &ref, Determinant &pd, MatrixXd &pPermutations) 
  {
    walkerVec.push_back(Walker<Jastrow, Slater>(corr, ref, pd));
    ...
    for (int i = 0; i < permutations.rows(); i++) {
      ...
      walkerVec.push_back(Walker<Jastrow, Slater>(corr, ref, dcopy));
    }
  };
 
  PermutedWalker(Jastrow &corr, const Slater &ref, MatrixXd &pPermutations) 
  {
    walkerVec.push_back(Walker<Jastrow, Slater>(corr, ref));
    vector<int> occA, occB;
  ...
  // used for debugging
  friend ostream& operator<<(ostream& os, const PermutedWalker& w) {
