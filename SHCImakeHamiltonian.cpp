...
void SHCImakeHamiltonian::SparseHam::makeFromHelper(
        HamHelpers2& helpers2, Determinant *SHMDets,
...
//=============================================================================
void SHCImakeHamiltonian::HamHelpers2::PopulateHelpers(
          Determinant* SHMDets,
          int DetsSize,
          int startIndex) {
//-----------------------------------------------------------------------------
  ...
  for (int i=0; i<connections.size(); i++) {
    ...
    for (int j=1; j<connections[i].size(); j++) {
      ...
      if (Determinant::Trev != 0)
        updateHijForTReversal(hij, Dets[i], Dets[connections[i][j]], I1, I2, coreE, orbDiff);
      Helements[i][j] = hij;
    }
  }
...
//-----------------------------------------------------------------------------
  ...
  if (commrank == 0) {
    for (int i=StartIndex; i<DetsSize; i++) {
      ...
      int sgn = 1;
      updateAlphaBeta(da, db, BetaN, AlphaN,
              AlphaMajorToBeta, AlphaMajorToDet,
              BetaMajorToAlpha, BetaMajorToDet,
              SinglesFromAlpha, SinglesFromBeta,
              Dets, DetsSize, StartIndex, sgn*(i+1));

      if (Determinant::Trev != 0 && Dets[i].hasUnpairedElectrons()) {
        int sgn = -1;
        updateAlphaBeta(db, da, BetaN, AlphaN,
            AlphaMajorToBeta, AlphaMajorToDet,
            BetaMajorToAlpha, BetaMajorToDet,
            SinglesFromAlpha, SinglesFromBeta,
            Dets, DetsSize, StartIndex, sgn*(i+1));
...
//=============================================================================
void SHCImakeHamiltonian::fixForTreversal(
        Determinant* Dets, int DetI, int DetJ,
        oneInt& I1, twoInt& I2, double& coreE,
...
//-----------------------------------------------------------------------------
  if (DetI < 0){
    ...
    if (!Dets[abs(DetJ)-1].hasUnpairedElectrons() && !Dets[abs(DetI)-1].hasUnpairedElectrons())
      return;
    if (!Dets[abs(DetJ)-1].hasUnpairedElectrons() || DetJ > 0){
      updateHijForTReversal(hij, Dets[std::abs(DetJ)-1], Dets[std::abs(DetI)-1], I1, I2, coreE, orbDiff);
    } else {
      if (Dets[std::abs(DetJ)-1].connected(Dets[std::abs(DetI)-1])){
        ...
        hij = Determinant::Trev*parity*Hij(dj, Dets[std::abs(DetI)-1], I1, I2, coreE, orbDiff);
        ...
        if (Dets[abs(DetI)-1].hasUnpairedElectrons()) {
          ...
          hij += Determinant::Trev*parity*Hij(Dets[std::abs(DetJ)-1], di, I1, I2, coreE, orbDiff);
          hij /= 2.0;
        } else {
          hij /= pow(2.0, 0.5);
        }
...
//-----------------------------------------------------------------------------
    ...
    :Inputs:
        ...
        bool diskio:
            BM_description
        SparseHam& sparseHam:
            The Hamiltonian in sparse form, composed of "Helements", "connections" and "orbDifference" (output)
        int Norbs:
            Number of orbitals (unused)
        oneInt& I1:
            One-electron tensor of the Hamiltonian
        twoInt& I2:
            Two-electron tensor of the Hamiltonian
...
//-----------------------------------------------------------------------------
  ...
  for (int i=0; i<AlphaMajorToBeta.size(); i++) {
    for (int ii=0; ii<AlphaMajorToBetaLen[i]; ii++) {
      ...
      if ( (std::abs(DetI)-1)%nprocs != proc || (std::abs(DetI)-1) < StartIndex || DetI < 0)
        continue;
      ...
      for (int j=0; j<SinglesFromAlphaLen[Astring]; j++) {
        ...
        int index = binarySearch( &BetaMajorToAlpha[Bstring][0], 0, BetaMajorToAlphaLen[Bstring]-1, Asingle);
        if (index != -1 ) {
          int DetJ = BetaMajorToDet[Bstring][index];
          if (std::abs(DetJ) >=  std::abs(DetI)) continue;
          size_t orbDiff;
          CItype hij = Hij(Dets[std::abs(DetJ)-1], Dets[std::abs(DetI)-1], I1, I2, coreE, orbDiff);
          fixForTreversal(Dets, DetI, DetJ, I1, I2, coreE, orbDiff, hij);
          if (abs(hij) >1.e-10) {
            connections[(std::abs(DetI)-offSet-1)/nprocs].push_back(std::abs(DetJ)-1);
            Helements  [(std::abs(DetI)-offSet-1)/nprocs].push_back(hij);
            if (DoRDM) orbDifference[(std::abs(DetI)-offSet-1)/nprocs].push_back(orbDiff);
          }
        }
      }
      ...
      for (int j=0; j< SinglesFromBetaLen[Bstring]; j++) {
        ...
        int index = binarySearch( &AlphaMajorToBeta[Astring][0], 0, AlphaMajorToBetaLen[Astring]-1, Bsingle);
...
//-----------------------------------------------------------------------------
  ...
  size_t totalMemory = 0,
         nBeta=BetaMajorToAlpha.size(), nAlpha =AlphaMajorToBeta.size();
...
void SHCImakeHamiltonian::updateSOCconnections(
        Determinant *Dets, int prevSize, int DetsSize,
    ...
    for (int ia=0; ia<nopen*nclosed; ia++){
      ...
      if (it != SortedDetsMap.end() ) {
        ...
        if (y < x) {
          ...
          //if (DoRDM) orbDifference[x/commsize].push_back(open[a]*norbs+closed[i]);
        }
      }
    } // ia
