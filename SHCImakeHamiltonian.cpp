...
void SHCImakeHamiltonian::HamHelpers2::MakeSHMHelpers() {
  SHCImakeHamiltonian::MakeSMHelpers( AlphaMajorToBeta, AlphaMajorToDet,
				      ...
				      DoublesFromAlpha, DoublesFromBeta,
				      ...
				      DoublesFromAlphaLen, DoublesFromAlphaSM,
				      DoublesFromBetaLen , DoublesFromBetaSM);
...
void SHCImakeHamiltonian::SparseHam::makeFromHelper(HamHelpers2& helpers2, Determinant *SHMDets, int startIndex, int endIndex, int Norbs, oneInt& I1, twoInt& I2, double& coreE, bool DoRDM) {
  ...
  SHCImakeHamiltonian::MakeHfromSMHelpers2(helpers2.AlphaMajorToBetaLen, 
					   ...
					   helpers2.DoublesFromAlphaLen, 
					   helpers2.DoublesFromAlphaSM ,

					   helpers2.SinglesFromBetaLen , 
					   helpers2.SinglesFromBetaSM  ,
					   helpers2.DoublesFromBetaLen , 
					   helpers2.DoublesFromBetaSM  ,
...
void SHCImakeHamiltonian::regenerateH(std::vector<Determinant>& Dets,
  ...
  int size = commsize, rank = commrank;
	...
	if (Determinant::Trev != 0) updateHijForTReversal(hij, Dets[i], Dets[connections[i][j]], I1, I2, coreE, orbDiff);
	Helements[i][j] = hij;
      }
    }
}
		     ...
		     int StartIndex, int DetIndex)
{
  ...
  if (itb == BetaN.end()) {
	...
	for (int l=0; l<j; l++)
	  for (int m=0; m<k; m++) {
	    HalfDet dbcopy1 = dbcopy;
	    dbcopy1.setocc(closedb[l], false);
	    dbcopy1.setocc(openb[m], true);
	    auto itbcopy = BetaN.find(dbcopy1);
	    //cout << j<<"  "<<k<<"  "<<l<<"  "<<m<<"  "<<dbcopy<<endl;
	    if (itbcopy != BetaN.end()) {
	      DoublesFromBeta[itb->second].push_back(itbcopy->second);
	      DoublesFromBeta[itbcopy->second].push_back(itb->second);
	    }
	  }
      }      
  }
  ...
  if (ita == AlphaN.end()) {
	...
	for (int l=0; l<j; l++)
	  for (int m=0; m<k; m++) {
	    HalfDet dacopy1 = dacopy;
	    dacopy1.setocc(closeda[l], false);
	    dacopy1.setocc(opena[m], true);
	    auto itacopy = AlphaN.find(dacopy1);
	    if (itacopy != AlphaN.end()) {
	      DoublesFromAlpha[ita->second].push_back(itacopy->second);
	      DoublesFromAlpha[itacopy->second].push_back(ita->second);
	    }
	  }
      }      
  }
					       ...
					       int StartIndex)
{
  ...
  if (commrank == 0) {
    for (int i=StartIndex; i<DetsSize; i++) {
      ...
      int sgn = 1;
      updateAlphaBeta(da, db, BetaN, AlphaN, AlphaMajorToBeta, AlphaMajorToDet, BetaMajorToAlpha, BetaMajorToDet, SinglesFromAlpha, SinglesFromBeta, DoublesFromAlpha, DoublesFromBeta, Dets, DetsSize, StartIndex, sgn*i);

      if (Determinant::Trev != 0 && Dets[i].hasUnpairedElectrons()) {
	int sgn = -1;
	updateAlphaBeta(db, da, BetaN, AlphaN, AlphaMajorToBeta, AlphaMajorToDet, BetaMajorToAlpha, BetaMajorToDet, SinglesFromAlpha, SinglesFromBeta, DoublesFromAlpha, DoublesFromBeta, Dets, DetsSize, StartIndex, sgn*i);
      }
    }
...
void SHCImakeHamiltonian::fixForTreversal(Determinant* Dets, int DetI, int DetJ, oneInt& I1, twoInt& I2, double& coreE, size_t& orbDiff, CItype& hij) {
  if (DetI < 0) hij = 0.0;
    ...
    if (!Dets[abs(DetJ)].hasUnpairedElectrons() && !Dets[abs(DetI)].hasUnpairedElectrons())
      return;
    if (!Dets[abs(DetJ)].hasUnpairedElectrons() || DetJ > 0) 
      updateHijForTReversal(hij, Dets[std::abs(DetJ)], Dets[std::abs(DetI)],
			    I1, I2, coreE, orbDiff);
    else {
      if (Dets[std::abs(DetJ)].connected(Dets[std::abs(DetI)])) //+ve is connected
	hij = 0.0;
      else {
	Determinant dj = Dets[std::abs(DetJ)];
	dj.flipAlphaBeta();
	double parity = Dets[std::abs(DetJ)].parityOfFlipAlphaBeta();
	hij = Determinant::Trev*parity*Hij(dj, Dets[std::abs(DetI)], I1, I2, coreE, orbDiff);
	if (Dets[abs(DetI)].hasUnpairedElectrons()) {
	  Determinant di = Dets[std::abs(DetI)];
	  di.flipAlphaBeta();
	  double parity = Dets[std::abs(DetI)].parityOfFlipAlphaBeta();
	  hij += Determinant::Trev*parity*Hij(Dets[std::abs(DetJ)], di, I1, I2, coreE, orbDiff);
	  hij /= 2.0;
	}
	else
	  hij /= pow(2.0, 0.5);
...
void SHCImakeHamiltonian::MakeHfromSMHelpers2(int*          &AlphaMajorToBetaLen, 
  ...
  for (size_t k=StartIndex; k<EndIndex; k++) {
    ...
    size_t orbDiff;
    if (Determinant::Trev != 0) updateHijForTReversal(hij, Dets[k], Dets[k], I1, I2, coreE, orbDiff);
  ...
  for (int i=0; i<AlphaMajorToBeta.size(); i++) {
	...
	DetI    = AlphaMajorToDet [i][ii];
      ...
      if (std::abs(DetI)%nprocs != proc || std::abs(DetI) < StartIndex || DetI < 0) 
	continue;
	...
	if (index != -1 ) {
	  ...
	  if (abs(hij) >1.e-10) {
	    connections[(std::abs(DetI)-offSet)/nprocs].push_back(std::abs(DetJ));
	    Helements  [(std::abs(DetI)-offSet)/nprocs].push_back(hij);
	    if (DoRDM) orbDifference[(std::abs(DetI)-offSet)/nprocs].push_back(orbDiff);
	  }
	}
      }
...
void SHCImakeHamiltonian::MakeSMHelpers(vector<vector<int> >& AlphaMajorToBeta,
  ...
  if (comm_rank == 0) {
    ...
    for (int i=0; i<nBeta; i++) {
      ...
      totalMemory       +=   sizeof(int) * DoublesFromBeta[i].size();
      BetaToAlphaTemp[i] = BetaMajorToAlpha[i].size();
      BetaSinglesTemp[i] = SinglesFromBeta[i].size();
      BetaDoublesTemp[i] = DoublesFromBeta[i].size();
    }
  }
  ...
  for (int i=0; i<nAlpha; i++) {
    ...
    DoublesFromAlphaSM[i] = begin + counter;  counter += DoublesFromAlphaLen[i];
  }
  ...
  for (int i=0; i<nBeta; i++) {
    ...
    DoublesFromBetaSM [i] = begin + counter;  counter += DoublesFromBetaLen [i];
  }
  ...
  if (comm_rank == 0) {
	...
	SinglesFromAlphaSM[i][j]  =  SinglesFromAlpha[i][j];
      for (int j=0; j<DoublesFromAlpha[i].size(); j++) 
	DoublesFromAlphaSM[i][j]  =  DoublesFromAlpha[i][j];
    }
	...
	SinglesFromBetaSM[i][j]  =  SinglesFromBeta[i][j];
      for (int j=0; j<DoublesFromBeta[i].size(); j++)  {
	DoublesFromBetaSM[i][j]  =  DoublesFromBeta[i][j];
      }
    }
  }
