...
void readIntegralsAndInitializeDeterminantStaticVariables(string fcidump) {
  //-----------------------------------------------------------------------------
  /*!
  // JETS: This function name is way too long
...
//-----------------------------------------------------------------------------
  ...
  // vector<int> irrep; // moved this to global variable
  ...
  if (commrank == 0) {
    I1.store.clear();
    I1.store.resize(2 * norbs * (2 * norbs), 0.0);
    I1.norbs = 2 * norbs;
    coreE = 0.0;
    ...
    // exit(0);
    I2.maxEntry =
        *std::max_element(&I2.store[0], &I2.store[0] + I2memory, myfn);
    I2.Direct = MatrixXd::Zero(norbs, norbs);
    I2.Direct *= 0.;
    I2.Exchange = MatrixXd::Zero(norbs, norbs);
    I2.Exchange *= 0.;
  ...
  MPI::COMM_WORLD.Bcast(&I2.store[(maxIter)*maxint],
                        I2memory - maxIter * maxint, MPI_DOUBLE, 0);
  world.barrier();
  ...
  for (int i = 0; i < norbs; i++) allorbs.push_back(i);
...
//-----------------------------------------------------------------------------
  ...
  if (commrank == 0) {
    std::map<std::pair<short, short>,
             std::multimap<float, std::pair<short, short>, compAbs> >::iterator
        it1 = I2.sameSpin.begin();
    for (; it1 != I2.sameSpin.end(); it1++)
      nonZeroSameSpinIntegrals += it1->second.size();

    std::map<std::pair<short, short>,
             std::multimap<float, std::pair<short, short>, compAbs> >::iterator
        it2 = I2.oppositeSpin.begin();
    for (; it2 != I2.oppositeSpin.end(); it2++)
      nonZeroOppositeSpinIntegrals += it2->second.size();

    std::map<std::pair<short, short>,
             std::multimap<float, short, compAbs> >::iterator it3 =
        I2.singleIntegrals.begin();
    for (; it3 != I2.singleIntegrals.end(); it3++)
      nonZeroSingleExcitationIntegrals += it3->second.size();
    ...
    memRequired +=
        nonZeroOppositeSpinIntegrals * (sizeof(float) + 2 * sizeof(short)) +
        ((norbs * (norbs + 1) / 2 + 1) * sizeof(size_t));
    memRequired +=
        nonZeroSingleExcitationIntegrals * (sizeof(float) + 2 * sizeof(short)) +
        ((norbs * (norbs + 1) / 2 + 1) * sizeof(size_t));
  }
  ...
  startingIndicesSameSpin =
      (size_t*)(startAddress + nonZeroSameSpinIntegrals * sizeof(float));
  sameSpinPairs =
      (short*)(startAddress + nonZeroSameSpinIntegrals * sizeof(float) +
               (norbs * (norbs + 1) / 2 + 1) * sizeof(size_t));

  oppositeSpinIntegrals =
      (float*)(startAddress +
               nonZeroSameSpinIntegrals * (sizeof(float) + 2 * sizeof(short)) +
               (norbs * (norbs + 1) / 2 + 1) * sizeof(size_t));
  startingIndicesOppositeSpin =
      (size_t*)(startAddress + nonZeroOppositeSpinIntegrals * sizeof(float) +
                nonZeroSameSpinIntegrals * (sizeof(float) + 2 * sizeof(short)) +
                (norbs * (norbs + 1) / 2 + 1) * sizeof(size_t));
  oppositeSpinPairs =
      (short*)(startAddress + nonZeroOppositeSpinIntegrals * sizeof(float) +
               (norbs * (norbs + 1) / 2 + 1) * sizeof(size_t) +
               nonZeroSameSpinIntegrals * (sizeof(float) + 2 * sizeof(short)) +
               (norbs * (norbs + 1) / 2 + 1) * sizeof(size_t));

  singleIntegrals =
      (float*)(startAddress + nonZeroOppositeSpinIntegrals * sizeof(float) +
               (norbs * (norbs + 1) / 2 + 1) * sizeof(size_t) +
               nonZeroSameSpinIntegrals * (sizeof(float) + 2 * sizeof(short)) +
               (norbs * (norbs + 1) / 2 + 1) * sizeof(size_t) +
               nonZeroOppositeSpinIntegrals * (2 * sizeof(short)));
  startingIndicesSingleIntegrals =
      (size_t*)(startAddress + nonZeroOppositeSpinIntegrals * sizeof(float) +
                (norbs * (norbs + 1) / 2 + 1) * sizeof(size_t) +
                nonZeroSameSpinIntegrals * (sizeof(float) + 2 * sizeof(short)) +
                (norbs * (norbs + 1) / 2 + 1) * sizeof(size_t) +
  ...
  if (commrank == 0) {
    ...
    for (int i = 0; i < norbs; i++)
      for (int j = 0; j <= i; j++) {
        std::map<std::pair<short, short>,
                 std::multimap<float, std::pair<short, short>, compAbs> >::
            iterator it1 = I2.sameSpin.find(std::pair<short, short>(i, j));

        if (it1 != I2.sameSpin.end()) {
          for (std::multimap<float, std::pair<short, short>,
                             compAbs>::reverse_iterator it =
                   it1->second.rbegin();
            ...
            sameSpinPairExcitations(it->second.first, it->second.second) +=
                abs(it->first);
    ...
    for (int i = 0; i < norbs; i++)
      for (int j = 0; j <= i; j++) {
        std::map<std::pair<short, short>,
                 std::multimap<float, std::pair<short, short>, compAbs> >::
            iterator it1 = I2.oppositeSpin.find(std::pair<short, short>(i, j));
        ...
        if (it1 != I2.oppositeSpin.end()) {
          for (std::multimap<float, std::pair<short, short>,
            ...
            oppositeSpinPairExcitations(it->second.first, it->second.second) +=
                abs(it->first);
            index++;
          }
        }
    ...
    for (int i = 0; i < norbs; i++)
      for (int j = 0; j <= i; j++) {
        std::map<std::pair<short, short>,
                 std::multimap<float, short, compAbs> >::iterator it1 =
            I2.singleIntegrals.find(std::pair<short, short>(i, j));

        if (it1 != I2.singleIntegrals.end()) {
          for (std::multimap<float, short, compAbs>::reverse_iterator it =
                   it1->second.rbegin();
               it != it1->second.rend(); it++) {
  ...
  MPI::COMM_WORLD.Bcast(shrdMem + (maxIter)*maxint,
                        memRequired - maxIter * maxint, MPI_CHAR, 0);
  world.barrier();
#endif

#ifndef SERIAL
  MPI::COMM_WORLD.Bcast(
      &sameSpinPairExcitations(0, 0),
      sameSpinPairExcitations.rows() * sameSpinPairExcitations.cols(),
      MPI_DOUBLE, 0);
  MPI::COMM_WORLD.Bcast(
      &oppositeSpinPairExcitations(0, 0),
      oppositeSpinPairExcitations.rows() * oppositeSpinPairExcitations.cols(),
      MPI_DOUBLE, 0);
#endif
...
void twoIntHeatBathSHM::getIntegralArray(int i, int j, const float*& integrals,
                                         ...
                                         size_t& numIntegrals) const {
  int I = i / 2, J = j / 2;
  int X = max(I, J), Y = min(I, J);

  int pairIndex = X * (X + 1) / 2 + Y;
  size_t start = i % 2 == j % 2 ? I2hb.startingIndicesSameSpin[pairIndex]
                                : I2hb.startingIndicesOppositeSpin[pairIndex];
  size_t end = i % 2 == j % 2 ? I2hb.startingIndicesSameSpin[pairIndex + 1]
                              : I2hb.startingIndicesOppositeSpin[pairIndex + 1];
  integrals = i % 2 == j % 2 ? I2hb.sameSpinIntegrals + start
                             : I2hb.oppositeSpinIntegrals + start;
  orbIndices = i % 2 == j % 2 ? I2hb.sameSpinPairs + 2 * start
                              : I2hb.oppositeSpinPairs + 2 * start;
