...
void AllocateSHM(vector<MatrixXx>& x0, CItype*& bcol, CItype*& sigmacol) {
  ...
  */
  ...
  regionDavidson = boost::interprocess::mapped_region{
      DavidsonSegment, boost::interprocess::read_write};
...
vector<double> davidson(Hmult2& H, vector<MatrixXx>& x0, MatrixXx& diag,
  ...
  */
  ...
  int nroots = x0.size();
  MatrixXx b;
  if (commrank == 0) b = MatrixXx::Zero(x0[0].rows(), maxCopies);
  ...
  while (true) {
...
#ifndef Complex
        if (commrank == 0)
          MPI_Reduce(MPI_IN_PLACE, sigmacol, brows, MPI_DOUBLE, MPI_SUM, 0,
                     shmcomm);
        else
          MPI_Reduce(sigmacol, sigmacol, brows, MPI_DOUBLE, MPI_SUM, 0,
                     shmcomm);
#else
        if (commrank == 0)
          MPI_Reduce(MPI_IN_PLACE, sigmacol, 2 * brows, MPI_DOUBLE, MPI_SUM, 0,
                     shmcomm);
        else
          MPI_Reduce(sigmacol, sigmacol, 2 * brows, MPI_DOUBLE, MPI_SUM, 0,
                     shmcomm);
#endif
      }  // localrank=0
      MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (commrank == 0) {
      ...
      // if (numIter < 2 && brows == 8) {
      //   std::cout << std::endl;
      //   std::cout << b.col(0) << std::endl << std::endl;
      //   std::cout << sigma.col(0) << std::endl << std::endl;
      //   std::cout << b.col(0).dot(sigma.col(0)) << std::endl;
      ...
      double error = r.norm();
      // if (numIter == 0)
      // if (print ) pout << str(boost::format("#niter:%3d root:%3d -> Energy :
      ...
      if (hsubspace.rows() == b.rows()) {
        ...
        for (int i = 0; i < x0.size(); i++) {
          ...
          if (print)
            pout << str(
                boost::format("#niter:%3d root:%3d -> Energy : %18.10g  \n") %
                (numIter) % (i) % eroots[i]);
        }
        continueOrReturn = 2;
        goto label1;
...
double LinearSolver(Hmult2& H, double E0, MatrixXx& x0, MatrixXx& b,
                    vector<CItype*>& proj, double tol, bool print) {
  //-----------------------------------------------------------------------------
  /*!
  Solve (H0-E0)*x0 = b
...
#ifndef SERIAL
    MPI_Allreduce(MPI_IN_PLACE, &Ap(0, 0), Ap.rows(), MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
#endif
    Ap = Ap - E0 * p;  // H0-E0
